...
namespace Microsoft.Partner.CSP.APISamples.SubscriptionStream
{
	class Stream
	{
		...
		public static dynamic CreateStream(DateTime startTime, string resellerCid, string sa_Token)
		{
			var body = new
			{
				start_time = startTime.ToString("MM/dd/yyyy hh:mm:ss"),
				page_size = 100
			};
			...
			try
			{
				...
				using (var reader = new StreamReader(response.GetResponseStream()))
				{
					...
					var createdStreamResponse = JsonConvert.DeserializeObject(responseContent);
		...
		public static dynamic DeleteStream(string resellerCid, string sa_Token)
		{
			...
			try
			{
				...
				using (var reader = new StreamReader(response.GetResponseStream()))
				{
					var responseContent = reader.ReadToEnd();
					Utilities.PrintWebResponse((HttpWebResponse)response, responseContent);
					var deletedStreamResponse = JsonConvert.DeserializeObject(responseContent);
		...
		/// <param name="sa_Token">sales agent token</param>
		public static void MarkStreamPageComplete(string completionHref, string httpMethod, string sa_Token)
		{
			var request = (HttpWebRequest)
				HttpWebRequest.Create(
			...
			try
			{
				...
				using (var reader = new StreamReader(response.GetResponseStream()))
				{
					var responseContent = reader.ReadToEnd();
					Utilities.PrintWebResponse((HttpWebResponse)response, responseContent);
					var pageCompleteResponse = JsonConvert.DeserializeObject(responseContent);
				}
			}
			catch (WebException webException)
			{
